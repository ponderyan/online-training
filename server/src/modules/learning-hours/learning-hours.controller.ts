...
export class LearningHoursController {
  constructor(private service: LearningHoursService, private resourceAccess: ResourceAccessService) {}
  ...
  async programStats(@Param('programId', ParseIntPipe) programId: number, @Req() req: any) {
  ...
  async getPendingHours(@Req() req: any, @Query('programId') programId?: string, @Query('source') source?: string) {
    return this.service.getPendingHours(programId ? parseInt(programId) : undefined, source, req.user?.orgId ?? null, req.user?.roles);
  }
  ...
  async review(
    ...
    await this.resourceAccess.assertLearningHourAccess(id, req.user?.orgId ?? null, req.user?.roles);

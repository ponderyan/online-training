...
import { OrgCodesService } from '../org-codes/org-codes.service.js';
...
export class OrganizationsService {
  constructor(private prisma: PrismaService, private orgCodes: OrgCodesService) {}
  ...
  async create(data: {
    ...
    // Phase 2: 若未提供 code，自动生成
    let autoGenerated = false;
    if (!data.code) {
      const rules = await this.orgCodes.getCodeRules();
      if (rules.autoGenerate) {
        data.code = await this.orgCodes.generateCode(data.parentId, data.name);
        autoGenerated = true;
      } else {
        throw new BadRequestException('组织编码不能为空（自动生成已关闭）');
      }
    }
    // 编码格式校验（仅手动输入时校验，自动生成跳过）
    if (!autoGenerated && !/^[A-Z][A-Z0-9-]{1,19}$/.test(data.code)) {

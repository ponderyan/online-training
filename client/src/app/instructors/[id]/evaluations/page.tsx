...
export default function InstructorEvaluationsPage() {
  ...
  return (
    <AppLayout>
      <button onClick={() => router.push(`/instructors/${params.id}`)} className="text-xs bg-transparent border-none cursor-pointer mb-4 text-[var(--fox)]" >← 返回讲师信息</button>
      ...
      {data ? (
              ...
              <tbody>
                {data.evaluations?.map((e: any, i: number) => (
                  <tr key={i}>
                    ...
                    <td className="text-[var(--ink-400)] text-xs max-w-[200px]">{e.comment || '—'}</td>
                    <td className="text-[var(--ink-300)] text-xs">{new Date(e.createdAt).toLocaleString('zh-CN')}</td>
                  </tr>
                ))}
                {(!data.evaluations || data.evaluations.length === 0) && <tr><td colSpan={5} className="text-[var(--ink-300)] text-center py-8 text-xs">暂无评价</td></tr>}
              </tbody>
            </table>
            </div>
          </div>
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-[var(--ink-300)]">暂无评价数据</p>
        </div>
      )}
    </AppLayout>
  );

    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget && !submitting) onClose(); }}>
      <div className="modal-card max-w-[560px] animate-fadeSlide">
        <div className="modal-header">
          ...
          <button onClick={onClose} disabled={submitting} className="text-[var(--ink-300)] text-lg bg-transparent border-none cursor-pointer">✕</button>
        </div>
        <div className="modal-body space-y-4">
          ...
          <div>
            <label className="text-[var(--ink-500)] block text-xs font-medium mb-1.5">正文内容 <span className="text-[var(--verm)]">*必填</span></label>
            <textarea value={content} onChange={e => setContent(e.target.value)}
              placeholder={'请粘贴教材正文内容…'} className="input textarea" rows={12} />
          </div>
          <div>
            <label className="text-[var(--ink-500)] block text-xs font-medium mb-1.5">出题要求 <span className="text-[var(--ink-400)] text-xs">（可选）</span></label>

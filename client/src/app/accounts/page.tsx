          ...
          <div className="bg-[var(--paper-bright)] w-[660px] max-h-[80vh] overflow-y-auto rounded-xl p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[var(--ink-700)] font-bold text-sm">{sideUser.displayName}</div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="bg-[var(--paper)] p-4 rounded-lg">
                ...
                <div className="text-xs space-y-1.5">
                  <div><span className="text-[var(--ink-300)]">姓名：</span>{sideUser.displayName}</div>
                  <div><span className="text-[var(--ink-300)]">性别：</span>{sideUser.gender === 'M' ? '男' : sideUser.gender === 'F' ? '女' : '—'}</div>
                  <div><span className="text-[var(--ink-300)]">身份证：</span>{sideUser.idCard || '—'}</div>
                  <div><span className="text-[var(--ink-300)]">手机：</span>{sideUser.phone || '—'}</div>
                  <div><span className="text-[var(--ink-300)]">邮箱：</span>{sideUser.email || '—'}</div>
                </div>
              </div>
              <div className="bg-[var(--paper)] p-4 rounded-lg">
                <div className="text-[var(--ink-400)] text-[10px] font-medium mb-2">教育/职称信息</div>
                <div className="text-xs space-y-1.5">
                  <div><span className="text-[var(--ink-300)]">学历：</span>{sideUser.education || '—'}</div>
                  <div><span className="text-[var(--ink-300)]">毕业院校：</span>{sideUser.educationSchool || '—'}</div>
                  <div><span className="text-[var(--ink-300)]">专业：</span>{sideUser.major || '—'}</div>
                  <div><span className="text-[var(--ink-300)]">毕业时间：</span>{sideUser.graduationDate ? new Date(sideUser.graduationDate).toLocaleDateString('zh-CN') : '—'}</div>
                  <div><span className="text-[var(--ink-300)]">职称：</span>{sideUser.professionalTitle || '—'} {sideUser.professionalLevel ? `(${sideUser.professionalLevel})` : ''}</div>
                </div>
              </div>
              <div className="bg-[var(--paper)] col-span-2 p-4 rounded-lg">
                <div className="text-[var(--ink-400)] text-[10px] font-medium mb-2">账号信息</div>
                <div className="text-xs space-y-1.5">
                  <div><span className="text-[var(--ink-300)]">角色：</span>{(sideUser.roles || [sideUser.role || 'STUDENT']).map((r: string) => ROLE_NAMES[r] || r).join('、')}</div>
                  <div><span className="text-[var(--ink-300)]">单位/职务：</span>{[sideUser.organization, sideUser.title].filter(Boolean).join(' · ') || '—'}</div>
                  <div><span className="text-[var(--ink-300)]">注册：</span>{new Date(sideUser.createdAt).toLocaleDateString('zh-CN')}</div>
                  <div><span className="text-[var(--ink-300)]">登录：</span>{sideUser.lastLoginAt ? new Date(sideUser.lastLoginAt).toLocaleString('zh-CN') : '从未'}</div>
                </div>
              </div>
            </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-2">
                      {sysRoles.length > 0 && (
                        <div>
                          <div className="text-[var(--ink-300)] text-[10px] font-medium mb-1">系统角色</div>
                      ...
                      {customRoles.length > 0 && (
                        <div>
                          <div className="text-[var(--ink-300)] text-[10px] font-medium mb-1">自定义角色（{customRoles.length}个）</div>
                          <div className="flex flex-wrap gap-1.5">

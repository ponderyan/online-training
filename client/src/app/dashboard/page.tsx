...
function StudentDashboard({ user, studentExams, studentStats, loading }: any) {
  ...
  if (loading) return <div className="text-center py-16" style={{ color: 'var(--ink-300)' }}>小狐狸正在加载… 🦊</div>;
    ...
    <>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="card p-4 text-center" style={{ border: activeExams.length > 0 ? '2px solid var(--fox)' : undefined }}>
            <div className="text-2xl font-bold" style={{ color: activeExams.length > 0 ? 'var(--fox)' : 'var(--ink-500)' }}>
              {activeExams.length}{activeExams.length > 0 && <span className="text-xs ml-1" style={{ color: 'var(--fox)' }}>进行中</span>}
            </div>
      ...
      {activeExams.length > 0 && (
        <div className="rounded-xl p-5 mb-6 flex items-center justify-between"
          style={{ background: 'linear-gradient(135deg, #fff3e0, #ffe0b2)', border: '2px solid var(--fox)' }}>
          <div className="flex items-center gap-4">
            <span className="text-3xl">📝</span>
            <div>
              <p className="font-bold text-sm" style={{ color: 'var(--ink-700)' }}>你有 {activeExams.length} 场考试正在进行</p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--ink-400)' }}>{activeExams[0].title} · 点击继续答题</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div className="card p-5">
          <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--ink-700)' }}>即将到来的考试</h3>
          {!nextExam ? (
            <p className="text-xs" style={{ color: 'var(--ink-300)' }}>暂无安排</p>
          ) : (
            <div>
              <p className="text-sm font-medium" style={{ color: 'var(--ink-700)' }}>{nextExam.title}</p>
              <p className="text-xs mt-1" style={{ color: 'var(--ink-400)' }}>{new Date(nextExam.startTime).toLocaleString('zh-CN')} · {formatCount(new Date(nextExam.startTime))}</p>
              <button onClick={() => router.push('/exam')} className="btn btn-outline btn-xs mt-3">查看全部考试 →</button>
            </div>
          )}
        </div>
        <div className="card p-5">
          <h3 className="text-sm font-semibold mb-4" style={{ color: 'var(--ink-700)' }}>学习进度</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs">
              <span style={{ color: 'var(--ink-500)' }}>已参加的考试</span>
              <span className="font-bold">{studentStats?.completed || 0} / {studentStats?.total || 0}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span style={{ color: 'var(--ink-500)' }}>学习中心</span>
              <button onClick={() => router.push('/learning-center')} className="text-xs bg-transparent border-none cursor-pointer" style={{ color: 'var(--fox)' }}>去学习 →</button>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span style={{ color: 'var(--ink-500)' }}>我的证书</span>
              <button onClick={() => router.push('/my-certificates')} className="text-xs bg-transparent border-none cursor-pointer" style={{ color: 'var(--fox)' }}>查看 →</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <>
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          { value: g.totalStudents ?? '—', label: '学员总数', icon: '👥', color: '#00897b' },
          { value: g.pendingGrading ?? '—', label: '待阅卷', icon: '📝', color: g.pendingGrading > 0 ? 'var(--fox)' : 'var(--sage)' },
          { value: g.pendingAppeals ?? '—', label: '待审核申诉', icon: '⚖️', color: g.pendingAppeals > 0 ? '#e53935' : 'var(--sage)' },
        ].map((s, i) => (
            ...
            <div className="space-y-2">
              {g.recentPrograms.map((p: any) => (
                <div key={p.id} onClick={() => router.push(`/programs/${p.id}`)}
                  ...
                  <span className="tag text-[10px] px-2 py-1 rounded-lg" style={{ background: `${STATUS_COLORS[p.status] || '#888'}18`, color: STATUS_COLORS[p.status] || '#888' }}>
        ...
        <div className="space-y-4">
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {(g.pendingGrading ?? 0) > 0 && (
                <div onClick={() => router.push('/grading')} className="flex items-center gap-2 p-2.5 rounded-lg cursor-pointer" style={{ background: 'rgba(232,122,48,0.06)' }}>
                  <span style={{ color: 'var(--fox)' }}>📝</span>
                  <span className="text-xs" style={{ color: 'var(--ink-600)' }}>{g.pendingGrading} 份阅卷待完成</span>
                </div>
              )}
              {(g.pendingAppeals ?? 0) > 0 && (
                <div onClick={() => router.push('/exams/appeals')} className="flex items-center gap-2 p-2.5 rounded-lg cursor-pointer" style={{ background: 'rgba(229,57,53,0.06)' }}>
                  <span style={{ color: '#e53935' }}>⚖️</span>
                  <span className="text-xs" style={{ color: 'var(--ink-600)' }}>{g.pendingAppeals} 个申诉待审核</span>
                </div>
              )}
              {(g.pendingCerts ?? 0) > 0 && (
                <div onClick={() => router.push('/certificates/applications')} className="flex items-center gap-2 p-2.5 rounded-lg cursor-pointer" style={{ background: 'rgba(232,122,48,0.06)' }}>
                  <span style={{ color: 'var(--fox)' }}>🏅</span>
                  <span className="text-xs" style={{ color: 'var(--ink-600)' }}>{g.pendingCerts} 份证书记审批</span>
                </div>
              )}
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={() => router.push('/admin/statistics')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>📊 数据中心</button>
            </div>
          </div>
        </div>
      </div>
...
function ExamOfficerDashboard({ stats, loading }: any) {
  ...
  return (
    <>
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          { value: d.totalQuestions ?? '—', label: '题库总量', icon: '📝', color: '#1565c0' },
          { value: d.totalPapers ?? '—', label: '试卷总数', icon: '📄', color: '#7b1fa2' },
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className="space-y-4 col-span-2">
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {(d.pendingGradingCount ?? 0) > 0 && (
                <div onClick={() => router.push('/grading')} className="flex items-center gap-2 p-2.5 rounded-lg cursor-pointer" style={{ background: 'rgba(232,122,48,0.06)' }}>
                  <span style={{ color: 'var(--fox)' }}>📝</span>
                  <span className="text-xs" style={{ color: 'var(--ink-600)' }}>{d.pendingGradingCount} 份试卷待阅卷</span>
                </div>
              )}
              {(d.pendingAppeals ?? 0) > 0 && (
                <div onClick={() => router.push('/exams/appeals')} className="flex items-center gap-2 p-2.5 rounded-lg cursor-pointer" style={{ background: 'rgba(229,57,53,0.06)' }}>
                  <span style={{ color: '#e53935' }}>⚖️</span>
                  <span className="text-xs" style={{ color: 'var(--ink-600)' }}>{d.pendingAppeals} 个申诉待审核</span>
                </div>
              )}
          ...
          {d.upcomingExams && d.upcomingExams.length > 0 && (
            <div className="card p-5">
              <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--ink-700)' }}>即将到来的考试</h3>
              <div className="space-y-2">
                {d.upcomingExams.map((e: any) => (
                  <div key={e.id} onClick={() => router.push(`/exams/${e.id}`)}
                    ...
                    <span className="text-sm font-medium" style={{ color: 'var(--ink-600)' }}>{e.title}</span>
        ...
        <div className="card p-5 self-start">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            <button onClick={() => router.push('/generate')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>✨ 智能组卷</button>
            <button onClick={() => router.push('/exams')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>📋 安排考试</button>
            <button onClick={() => router.push('/grading')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>📊 阅卷中心</button>
          </div>
        </div>
      </div>
    </>
  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { value: d.myQuestions ?? '—', label: '我的试题', icon: '📝', color: '#1565c0' },
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        <div className="card p-5 col-span-2">
          ...
          <div className="space-y-2">
            {(d.pendingGradingCount ?? 0) > 0 && (
              <div onClick={() => router.push('/grading')} className="flex items-center gap-2 p-2.5 rounded-lg cursor-pointer" style={{ background: 'rgba(232,122,48,0.06)' }}>
                <span style={{ color: 'var(--fox)' }}>📝</span>
                <span className="text-xs" style={{ color: 'var(--ink-600)' }}>{d.pendingGradingCount} 份试卷待批阅</span>
              </div>
            )}
            {(d.pendingGradingCount ?? 0) === 0 && <p className="text-xs" style={{ color: 'var(--ink-300)' }}>暂无待办事项 ✅</p>}
          </div>
        </div>
        ...
        <div className="card p-5 self-start">
          ...
          <div className="grid grid-cols-1 gap-2">
            <button onClick={() => router.push('/questions')} className="btn btn-fox btn-xs">📝 录入试题</button>
            <button onClick={() => router.push('/materials')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>📖 教材出题</button>
            <button onClick={() => router.push('/grading')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>📊 阅卷</button>
          </div>
        </div>
      </div>
    </>
    ...
    <>
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="card p-5">
          ...
          <div className="text-xs mt-1" style={{ color: 'var(--ink-400)' }}>进行中的考试</div>
        </div>
      ...
      {d.upcomingExams && d.upcomingExams.length > 0 && (
        <div className="card p-5 mb-8">
          <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--ink-700)' }}>即将开始的考试</h3>
          <div className="space-y-2">
            {d.upcomingExams.map((e: any) => (
              <div key={e.id} className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'var(--paper-dark)' }}>
                <span className="text-sm" style={{ color: 'var(--ink-600)' }}>{e.title}</span>
                <span className="text-xs" style={{ color: 'var(--ink-300)' }}>
                  {new Date(e.startTime).toLocaleString('zh-CN', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
...
function AgencyAdminDashboard({ stats, loading }: any) {
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[
          { value: d.totalStudents ?? '—', label: '名下学员', icon: '👥', color: '#00897b' },
      ...
      <div className="card p-5 mb-8">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => router.push('/admin/learning-hours-review')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>⏱ 学时管理</button>
        </div>
      </div>
    </>
  );
    ...
    <>
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[
          ...
          { value: d.totalLogCount ?? '—', label: '累计日志', icon: '📦', color: '#7b1fa2' },
        ].map((s, i) => (
          <div key={i} className="card p-5 flex items-center gap-4">
            <span className="text-3xl">{s.icon}</span>
            <div>
      ...
      {d.recentLogs && d.recentLogs.length > 0 && (
        <div className="card p-5 mb-8">
          <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--ink-700)' }}>最近操作记录</h3>
          <div className="space-y-1">
            {d.recentLogs.map((log: any) => (
              <div key={log.id} className="flex items-center gap-3 p-2 rounded text-xs" style={{ color: 'var(--ink-500)' }}>
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: 'var(--fox)' }} />
                <span>{log.action}</span>
                <span className="text-[10px]" style={{ color: 'var(--ink-300)' }}>{log.entityType || '—'}</span>
      ...
      <div className="card p-5">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => router.push('/audit-logs')} className="btn btn-sm btn-xs" style={{ border: '1px solid var(--ink-200)' }}>📋 审计日志</button>
        </div>
      </div>
    </>
  );
    ...
    <AppLayout>
      <div className="mb-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg" style={{ color: 'var(--fox)' }}>{user?.displayName ? `，${user.displayName}` : ''}</span>
        </div>

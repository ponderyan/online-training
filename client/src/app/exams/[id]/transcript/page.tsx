...
export default function ExamTranscriptPage() {
  ...
  if (!data) return <AppLayout><div className="text-[var(--ink-300)] text-center py-16">暂无成绩数据</div></AppLayout>;
    ...
    <AppLayout>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          <div key={i} className="card p-5 text-center">
            ...
            <div className="text-[var(--ink-300)] text-xs mt-1">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        {data.scores?.map((s: any, i: number) => (
          <div key={i} className="border-[var(--ink-100)] flex items-center gap-4 px-5 py-3 border-b last:border-b-0">
            <span className="text-sm w-8 text-center" style={{ color: i < 3 ? 'var(--gold)' : 'var(--ink-300)' }}>
              {i < 3 ? ['🥇', '🥈', '🥉'][i] : `#${i + 1}`}
            </span>
            <span className="text-[var(--ink-600)] flex-1 text-sm font-medium">{s.student?.displayName || '—'}</span>
            <span className="text-[var(--ink-500)] text-sm font-mono">{s.finalScore ?? '-'} 分</span>
        ...
        {(!data.scores || data.scores.length === 0) && (
          <div className="text-[var(--ink-300)] px-5 py-8 text-center text-xs">暂无数据</div>
        )}
      </div>
    </AppLayout>
  );
